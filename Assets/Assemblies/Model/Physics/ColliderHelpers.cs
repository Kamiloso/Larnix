...
public static class ColliderHelpers
{
    ...
    static ColliderHelpers()
    {
        if (MaxColliderSize.x <= 0 || MaxColliderSize.x >= BlockHelpers.CHUNK_SIZE ||
            MaxColliderSize.y <= 0 || MaxColliderSize.y >= BlockHelpers.CHUNK_SIZE)
        {
    ...
    {
        Vec2Int entityChunk = BlockHelpers.CoordsToChunk(position);
