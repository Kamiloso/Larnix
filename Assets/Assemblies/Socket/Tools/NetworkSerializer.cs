...
internal static class NetworkSerializer
{
    ...
    public static byte[] ToBytes<T>(
        in PayloadHeader header, in T payload, IKey? key) where T : unmanaged
    {
    ...
    public static bool TryDecryptNetworkBytes(byte[] bytes, IKey? key, out byte[] decrypted)
    {
        key ??= KeyEmpty.Instance;
    ...
    public static bool TryNetworkBytesAs<T>(
        byte[] bytes, IKey? key, out PayloadHeader header, out T payload) where T : unmanaged
    {
        key ??= KeyEmpty.Instance;
