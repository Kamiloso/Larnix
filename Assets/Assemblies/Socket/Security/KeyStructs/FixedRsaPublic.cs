...
namespace Larnix.Socket.Security.KeyStructs;
...
internal readonly record struct FixedRsaPublic : ISanitizable<FixedRsaPublic>
{
    ...
    public byte[] Bytes264 => ArrayUtils.MegaConcat(
        _bufferPublicRsa1.ToArray(),
        _bufferPublicRsa2.ToArray()
        ); // public - no need to clean after use
    ...
    {
        byte[] bytes264 = Bytes264;
        FixedRsaPublic sanitized = new(bytes264);
        Array.Fill(bytes264, (byte)0);
        return sanitized;
    }
}
