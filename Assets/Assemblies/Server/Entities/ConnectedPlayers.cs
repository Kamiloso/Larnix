...
internal interface IConnectedPlayers
{
    ...
    void JoinPlayer(string nickname, IPEndPoint endpoint);
    ...
    JoinedPlayer this[string nickname] => GetPlayer(nickname)!;

    string NicknameByUid(ulong uid);
    ulong UidByNickname(string nickname);

    PlayerState StateOf(string nickname) => GetPlayer(nickname)?.GetState() ?? PlayerState.None;
...
internal class ConnectedPlayers : IConnectedPlayers
{
    ...
    public void JoinPlayer(string nickname, IPEndPoint endpoint)
    {
        if (_players.ContainsKey(nickname))
            throw new InvalidOperationException($"Player with nickname {nickname} is already connected.");

        ulong uid = (ulong)UserRepository.FindByNickname(nickname)!.Uid;
        var player = new JoinedPlayer(uid, nickname, endpoint);
