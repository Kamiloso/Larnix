...
namespace Larnix.Socket.Frontend
{
    public class Client : IDisposable
    {
        ...
        public static Task<Client> CreateClientAsync(string address, string authcode, string nickname, string password)
        {
            return Task.Run(() =>
            {
                IPEndPoint endPoint = Resolver.ResolveString(address);
                ...
                try
                {
                    return new Client(endPoint, ticket, authcode, nickname, password);
                }
                catch
                {
                    return null;
        ...
        private Client(IPEndPoint endPoint, EntryTicket ticket, string authcode, string nickname, string password)
        {
            EndPoint = endPoint;
